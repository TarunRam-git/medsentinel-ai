...
function key() {
  ...
  if (!value || !/^[a-f0-9]{64}$/i.test(value))
    throw new Error(
      "DATA_ENCRYPTION_KEY must be a 32-byte hexadecimal secret. Run npm run setup.",
    );
...
export const auditHash = (value: string) =>
...
export function verifyPassword(value: string, stored: string) {
  ...
  return timingSafeEqual(scryptSync(value, salt, 64), expected);
}
